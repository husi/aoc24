...
auto parse(std::string_view input)
{
  ...
  auto pipeline =
      ...
      std::views::transform([](auto c) -> unsigned int
                            { return c - '0'; }) |
...
NodeList::iterator distribute_last(NodeList &nodes, NodeList::iterator start)
{
  ...
  while (last.length > 0)
  {
    ...
    else
    {
      ...
      start = nodes.insert(++start, Node(-1, gap.index + last.length, remainder));
...
{
  return std::ranges::fold_left_first(nodes | std::views::transform([](auto node)
